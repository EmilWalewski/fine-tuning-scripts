...
def extract_clean_markdown(pdf_path: str) -> str:
    ...
    out_pages = []
    rescued_n = fallback_n = dropped_n = 0
        ...
        if _has_value_jam(md):
            ...
            if _validate_rescue(rows):
                ...
                if fixed_blocks and not _has_value_jam(repaired):
                    ...
                    dropped_n += 1
        out_pages.append(md)
    if rescued_n or fallback_n or dropped_n:
        print(f"    [tier-2] strony z jamem: rescue={rescued_n}, fallback={fallback_n}, pominięte_tabele={dropped_n}")
    return "\n\n".join(out_pages)
